...
const LoginPage = () => {
  ...
  const searchParams = new URLSearchParams(router.query.search);
  ...
  useEffect(() => {
    ...
    const get = async () => {
      if (user) {
        ...
        if (!userDoc.exists()) {
          // User is new, store their name in Firestore
  ...
  return (
    <div className="w-screen h-screen flex items-center justify-center">
      {initialLoading ? (
        ...
        <>
          {initialError !== "" ? (
            <div className="text-error">{initialError}</div>
          ) : (
            <>
              ...
              {user ? (
                ...
                <div className={styles.formcontainer}>
                  ...
                  <form className={styles.form} onSubmit={handleSignIn}>
                    <div className={`${styles.inputgroup} mb-5`}>
                    ...
                    <button className={styles.sign}>
                      {loginLoading ? "Logging you in" : "Sign in with Email"}
                    </button>
                    ...
                    {/* show info msg */}
                    {infoMsg !== "" && (
                      <div className="text-success">{infoMsg}</div>
                    )}
                  </form>
